	...
	constructor( initialState, handlers, ...mwares ) {
		...
		const _dispatch = function ( action ) {
			if ( _handlers[action.type] ) {
				let trans;
				if ( _handlers[action.type].jsonpath ) {
					trans.jsonpath = Store.clone(_handlers[action.type].jsonpath);
					trans = _handlers[action.type](action, Store.clone( path.query( _state, trans.jsonpath ) ) );
					if (trans.newState) {
						path.query( _state, _handlers[action.type].jsonpath ).forEach( function ( match ) {
							match = Store.clone(trans.newState);
						});
						_state = Store.clone(trans.newState);
					}
				} else {
					trans = _handlers[action.type](action, Store.clone(_state));
					if (trans.newState) {
						_state = Store.clone(trans.newState);
					}
				}
